...
1. **DC-16 pgTAP harness** — stand up first; port behavioral checks; add invariant tests (RLS coverage V5, money-currency, append-only). *Precondition for the rest of Batch 0.*
...
A1 (RLS init-plan wrapping) · A2 (18 tenant_id indexes) · B5 (DML grants + anon scope) + **DC-5 storage RLS** · **DC-15 service_role bounding** · B2 (CHECK constraints) · B6 (naming normalization) · B1 (reference-data integrity) · **OPS-1** (structured logging/metrics/tracing + documented RPO/RTO).
