-- Migration: gate_booking_item_execution
-- Phase 5 (Booking Core) — Finance Approval Gate, execution-approval slice (ADR-0020), step 3 of 3.
-- Adds the finance precondition to the confirmed -> in_progress edge of app.advance_booking_item (the
-- integration point already marked in migration 202607045700). Execution is blocked unless the item is
-- either not finance-gated (finance_approval_required = false) or has an approved finance approval
-- (finance_approval_status_code = 'approved', set by app.review_finance_approval). This CLOSES the
-- execution-approval gate: request (SPEC-081) -> review/approve+lock (SPEC-082) -> execute (here).
-- create-or-replace of the existing function (applied migrations are immutable); the ONLY changes vs
-- 202607045700 are (a) selecting the two finance columns and (b) the gate check after authorization.
-- All other transitions and side effects are unchanged. Negative-balance risk flag remains deferred to
-- Finance Core (app.customer_balance()), per ADR-0020.
...
begin
    ...
    select bi.id, bi.service_type_code, bi.base_status_code, bi.is_archived,
           bi.finance_approval_required, bi.finance_approval_status_code
      into v_item
    ...
    -- FINANCE GATE (ADR-0020): confirmed -> in_progress begins execution. If the item is finance-gated,
    -- an approved finance_execution_approval must exist. Ungated items (finance_approval_required=false)
    -- pass through unchanged.
    if v_item.base_status_code = 'confirmed' and p_to_status = 'in_progress'
       and v_item.finance_approval_required
       and coalesce(v_item.finance_approval_status_code, '') <> 'approved' then
        raise exception 'execution blocked: finance approval is required and not approved for this booking item';
    end if;
