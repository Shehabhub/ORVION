-- Migration: advance_booking_refund_reissue
-- Phase 6 (Finance Core). Booking-level lifecycle -- ADR-0020 capability set, slice 5 (Refund/Reissue),
-- the FINAL slice: after it the entire 26 Booking State Machine is implemented.
--   issued   -> refunded   (booking_refunded,        REFUND_BOOKING)
--   issued   -> reissue    (booking_reissue_started, REISSUE_BOOKING)
--   reissue  -> issued     (booking_issued,          ISSUE_BOOKING)   -- re-issuance IS issuance
--   refunded -> completed  (booking_completed,       CREATE_BOOKING)
-- Refund and Reissue are finance-consequential capabilities and mint REFUND_BOOKING / REISSUE_BOOKING
-- (owner/ceo/branch_manager/finance_manager). Completing the re-issuance (reissue -> issued) reuses
-- ISSUE_BOOKING -- every transition INTO 'issued' is issuance and therefore also runs the negative-balance
-- risk-flag check (ADR-0020), so re-issuing before full collection is gated and flagged exactly like a
-- first issuance, with no extra code. refunded -> completed is booking-operator completion (CREATE_BOOKING).
--
-- create-or-replace of app.advance_booking (applied migrations are immutable): changes vs 202607046700 are
-- (a) the four new transition rows, (b) 'refunded'/'reissue' added to the 'warning' severities, and
-- (c) REMOVING the deferred-contract in-list entirely -- no booking transition is deferred any longer, so an
-- unknown target simply falls through to "transition not allowed". Issuance risk-flag logic, the
-- cancellation-reason guard, and all other transitions/side effects are unchanged.

insert into permissions (key, name, is_system, is_active)
values ('REFUND_BOOKING', 'Refund Booking', true, true),
       ('REISSUE_BOOKING', 'Reissue Booking', true, true)
...
where p.key in ('REFUND_BOOKING', 'REISSUE_BOOKING')
...
begin
    ...
    -- Negative-balance issuance risk flag (ADR-0020): on any transition INTO 'issued' (first issuance or
    -- re-issuance), snapshot the customer's per-currency balance for this booking; if any currency shows the
    -- customer still owes (outstanding > 0), issuing before full collection requires
    ...
    -- 'warning' (27); issuance and progress/completion are 'info'.
