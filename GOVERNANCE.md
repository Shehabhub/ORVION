...
Version 1.6 · 2026-07-15 · Governs every future human, Claude, Codex, and AI/MCP session.
...
|---|---|---|---|
| **Governance-lint hook** (custom, in `scripts/` + Claude PostToolUse hook) | enforces: every report has a class header; no finding ID has conflicting status across Masters; `manifest.md Active CR` cleared after Complete | **High** | **PARTIALLY IMPLEMENTED (2026-07-15):** `scripts/check_repository_consistency.ps1` + the `Repository Consistency` CI workflow now enforce (a) no broken document references in Living docs and (b) no intra-register finding-status contradiction — the two drift classes the recovery repaired. Remaining extensions (report class-header presence; `Active CR` cleared after Complete; a local PostToolUse hook for pre-commit feedback) are additive to the same script. |
