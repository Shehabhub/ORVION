...
Last Completed Task: SPEC-067 — Phase 4 (CRM Core) lead SLA escalation + first scheduled workload under ADR-0018 (scheduler-agnostic background processing: logic→RPCs, scheduling→fittest scheduler [today pg_cron], external I/O→Edge/n8n). app.process_lead_sla(warn_after, reassign_after) SECURITY DEFINER service_role system scan: at warn threshold emits lead_sla_warning + notifies assignee/managers; at reassign threshold reassigns to a different eligible member (round-robin, history preserved) + lead_reassigned event + notifies new assignee. Scheduled every minute via pg_cron (lead-sla-processor). Proven E2E via backdated timestamps: warn-once (no double), reassign to Ed (2 history rows/1 current), service_role-only. Smoke-test passes. (Prior: SPEC-066 interaction recording; SPEC-065 round-robin+event seam.)
