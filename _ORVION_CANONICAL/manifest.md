...
Last Completed: Phase-8 core slice — migration `202607049200` per ADR-0023: `orvion_integration` role (NOLOGIN; operator enables + sets password out-of-repo), `app.record_offline_conversion` (MANAGE_MARKETING_CAMPAIGN-gated, catalog-validated, emits `offline_conversion_created`), and the n8n outbox pair — `claim_conversion_deliveries` (SECURITY DEFINER; consent-granted-only via SPEC-119 columns; SKIP LOCKED; failed→retried bookkeeping; retry ceiling 5; returns identifiers for edge hashing) + `record_conversion_delivery_result` (pending→sent/failed + canonical events). Verified: reset clean, smoke `ALL CHECKS PASSED (67/565)`, full behavioral lifecycle (claim/idempotent-reclaim/ack/no-reclaim/consent-denial/grants) green.

Next capability: **Phase 8 — automated outcome→conversion mapper**: a scheduled `SECURITY DEFINER` RPC (pg_cron per ADR-0018) consuming `events` via the `seq` cursor, mapping canonical outcomes (canon 21: `qualified_lead`/`booking_created`/`payment_received`/`ticket_issued`) to `offline_conversions` rows (value = revenue) for leads with attribution clicks. Then attribution-click capture RPC (`capture_attribution_click`), then the n8n workflow (Data Manager API + ECL; requires owner's Google OAuth `datamanager` credentials + operator-set `orvion_integration` password). Phase 9 Tier B stays evidence-gated (ADR-0022).
