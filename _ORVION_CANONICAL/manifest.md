...
Last Completed Task: SPEC-043 — migration 13 (event and notification tables): created events, security_events, notifications, notification_deliveries per 31 section 7; polymorphic entity fields (no FK), nullable platform-level tenant_id on events/security_events, security_events.ip_address inet, payload jsonb, no updated_at triggers; verified on a clean local database (platform-event null-tenant insert + notification FK restrict). DB-enforced event immutability deferred to RLS migration 19
