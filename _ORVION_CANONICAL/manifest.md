...
Last Completed Task: SPEC-052 — migration 19 (RLS policies): enabled RLS on all 71 public base tables with 76 policies derived from 35; app.current_tenant_id() SECURITY DEFINER resolution primitive in the non-API app schema (active-tenant-aware, degrades to single active membership); tenant_isolation on 54 tenant tables, read-all on 10 globals, own-tenant on tenants, auth.uid() ownership on 3 auth-support tables, catalog_values dual-read, append-only events/security_events with immutability trigger. Verified on clean reset; row-scoping + default-deny + immutability proven behaviorally. Finding logged: authenticated lacks DML grants (backend-phase concern; current state safe). The database foundation (schema + seed + RLS) is now COMPLETE
