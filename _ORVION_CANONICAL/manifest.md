...
Governance update (2026-07-15): owner issued a Delegation & Model-Selection policy that **supersedes** the earlier "delegate whenever it increases quality / cost is not a concern" stance (checkpoint §1.4). New rule: delegation is NOT the default — delegate only when the work is genuinely parallelizable AND needs external/specialized research beyond current repository context AND reduces time without reducing quality AND cannot be done as well alone; justify each spawn (why necessary, why not self, unique value, expected evidence); minimum agents, no overlap; choose the smallest capable model (no expensive reasoning models for routine inspection/indexing/formatting/sync/comparisons — reserve for architecture validation, complex dependency/cross-doc synthesis, hard research, critical design); validate returned evidence yourself, reconcile conflicts, never trust blindly. Same governance/Earn-It applies to every delegated agent. To be persisted into AGENTS.md (owner-gated) as part of the Track-2 package.
