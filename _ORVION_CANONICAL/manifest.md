...
Last Completed: Phase-8 outcome→conversion mapper — migration `202607049300`: `app.map_outcomes_to_conversions` (SECURITY DEFINER, integration-role-only), the **first consumer of the `events.seq` cursor** — maps `lead_qualified`/`booking_created`/`payment_recorded`/`booking_issued` → canon-21 conversion types (payment carries revenue value); only leads with a first-touch `attribution_click_id`; idempotent via `offline_conversions.source_event_seq` provenance (unique) + `integration_cursors` watermark (moddatetime-triggered per ADR-0008); consent stays solely the claim RPC's gate (One Authority). **No pg_cron**: n8n invokes map→claim→send→ack as one workflow (ADR-0018/0023). Verified: reset clean, smoke `ALL CHECKS PASSED (72 tables)`, behavioral suite green (4 mapped incl. revenue 1500 USD, click-less lead ignored, re-run 0, cursor advanced, grants exact). Prior slice: migration `049200` (outbox claim/ack + integration role).

Next capability: **Phase 8 — attribution-click capture RPC** (`app.capture_attribution_click`: landing-page ingestion writing `attribution_clicks` + linking/creating the lead anchor, canon 21 §Captured Click Data), then the **n8n workflow** (map→claim→send→ack against the Google Data Manager API + ECL; requires the owner's Google OAuth `datamanager` credentials and an operator-set `orvion_integration` password + login enable). Phase 9 Tier B stays evidence-gated (ADR-0022).
