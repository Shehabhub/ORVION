...
Last Completed Task: SPEC-044 — migration 14 (subscription tables): created subscription_plans, feature_entitlements (both global, no tenant_id), subscriptions, subscription_payment_proofs, usage_counters per 31 section 8; plain-text status/plan/feature/metric codes, date period columns, numeric value columns, updated_at triggers on subscriptions/usage_counters; verified on a clean local database (document_id FK restrict + subscriptions moddatetime). Subscription business-key uniqueness logged as a Recommended backlog item
